...
def resolve_disputed_pixels_by_proximity(
    ...
    """
    Raster-based post-processing for OVERLAPPING regions -- returns
    POLYGONS (one np.ndarray of [x, y] vertices per region), same in/out
    convention as `close_contiguous_gaps`.
 
    Unlike `close_contiguous_gaps` (which pulls shared VERTICES together),
    this ignores the input vertex ordering and instead resolves overlap on
    a pixel grid, by ownership, then traces new region boundaries out of
    the resolved raster (marching squares):
 
      * A pixel covered by exactly one region's polygon keeps that region
        ("undisputed").
      * A pixel covered by two or more regions' polygons is "disputed". It
        is handed to whichever contending region has its nearest
        UNDISPUTED pixel closest to it (Euclidean distance, in pixels).
        i.e. the disputed strip is absorbed by whichever region's solid,
        uncontested body it sits closer to.
      * Exact ties are broken by `region_order` — the region listed
        earliest wins. If a contending region has NO undisputed pixels at
        all (its entire claimed area is disputed), it has no solid body to
        measure from and loses any tie to a region that does; if none of
        the contenders have one, the tie-break falls back to plain
        `region_order` priority.
 
    The result is a contiguous, non-overlapping raster labelling, useful
    for rendering or for driving contour extraction later — it does not
    modify `polygons` itself.
 
    Parameters
    ----------
    polygons : list[np.ndarray]
        One (n_i, 2) array of [x, y] vertices per region, all in the same
        coordinate space (e.g. optimised output of CartogramFramework_global).
    resolution : int
        Pixel count along the LONGER side of the combined bounding box; the
        shorter side is scaled to keep pixels square. Higher = crisper
        boundaries, slower / more memory.
    region_order : Optional[list[int]]
        Tie-break priority as a list of region indices, earliest = highest
        priority. Defaults to [0, 1, 2, ...] (i.e. input order — region A
        beats B beats C, matching the "prefer A over B over C" request).
    bounds : Optional[tuple]
        (xmin, ymin, xmax, ymax) to rasterise over. Defaults to the
        combined bounding box of all polygons, with a small margin.
    simplify_tolerance : float
        If > 0, simplifies each traced boundary with
        skimage.measure.approximate_polygon (Douglas-Peucker) at this
        tolerance, in the SAME units as `polygons` (not pixels). Marching
        squares produces roughly one vertex per pixel the boundary
        crosses, so for resolution=500 a region can easily come back with
        several thousand points; simplifying is usually worth doing.
        0 (default) = no simplification, raw traced boundary.
    return_raster : bool
        If True, return (polygons, raster_info) where raster_info is the
        dict of {"label_grid", "coverage_count", "extent", "pixel_size"}
        for debugging/visualisation. If False (default), return just the
        polygons list.
 
    Returns
    -------
    list[np.ndarray]  — new list of polygons (one per input region, same
    order), traced from the resolved raster. If a region never rasterises
    at the given `resolution`, or ends up losing every pixel to
    higher-priority neighbours (fully swallowed, no undisputed core of its
    own), it is NOT returned as an empty array -- it falls back to its
    ORIGINAL (pre-resolution) polygon instead, and a warning is issued
    naming how many regions this happened to. This keeps every entry a
    valid, non-empty polygon (so downstream code like `polygon_areanp(p)`
    or plotting never has to special-case an empty region), at the cost of
    that region not actually being disputed-resolved -- raise `resolution`
    or check `region_order` if you see the warning and want a "true" fix
    instead of the fallback.
    If `return_raster=True`, returns (polygons, raster_info) instead.
    """
