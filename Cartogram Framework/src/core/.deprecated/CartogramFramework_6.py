...
def CartogramFramework_global(
    ...
    _allow_contiguous_relaxation_retry: bool = True,
):
    # ------------------------------------------------------------------
    # 0. Preprocessing
    # ------------------------------------------------------------------
    ...
    try:
        ...
        prob.status = f"solver_error: {exc}"
    ...
    # If the hard contiguous solve fails, retry once with the same hard
    # contiguous model but with the constraints that most often trigger
    # infeasibility relaxed: soft mean-scale, looser t/u boxes, and no
    # topology ordering. This keeps exact shared-vertex locking intact.
    solver_ok = prob.status in (
        cp.OPTIMAL, cp.OPTIMAL_INACCURATE,
        "optimal", "optimal_inaccurate",
    )
    if not solver_ok and shape == "contiguous" and _allow_contiguous_relaxation_retry:
            print(
                "  WARNING: hard contiguous solve failed; retrying with a "
                "relaxed hard-contiguous pass."
            )
            return CartogramFramework_global(
                ...
                lambda_center=lambda_center,
                lambda_topology=lambda_topology,
                lambda_contiguous=lambda_contiguous,
                contiguous_area_ratio_cap=contiguous_area_ratio_cap,
                contiguous_use_topology=False,
                contiguous_skip_hard_ordering_for_adjacent=contiguous_skip_hard_ordering_for_adjacent,
                gamma=gamma,
                beta=beta,
                alpha=alpha,
                epsilon=epsilon,
                ...
                postprocess_snap_method=postprocess_snap_method,
                compute_leaders_flag=compute_leaders_flag,
                leader_tol=leader_tol,
                soft_mean_scale=True,
                lambda_mean_scale=min(lambda_mean_scale, 1e3),
                lambda_repulsion=lambda_repulsion,
                repulsion_only_adjacent=repulsion_only_adjacent,
                repulsion_margin=repulsion_margin,
                lambda_vertex_distance=lambda_vertex_distance,
                vertex_distance_margin=vertex_distance_margin,
                lambda_angle=lambda_angle,
                lambda_anchor=lambda_anchor,
                u_max=max(u_max, 0.5),
                beta_u=beta_u,
                postprocess_resolve_overlaps=True,
                _allow_contiguous_relaxation_retry=False,
            )
    ...
    if not solver_ok:
        print(
            ...
            f"    • Increase u_max / t_max for harder shared-vertex cases"
        )
