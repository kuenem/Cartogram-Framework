...
def explain_spatial_deformation():
    fig, ax = _new_ax(figsize=(6.5, 5.5))# , title="Spatial deformation: has each region drifted from its original place?")
...
# ----------------------------------------------------------------------
# 9. Local shape preservation — Section 2.4, criterion 7
# (do distinctive local features of one region — a notch, a peninsula, a
# sharp corner — survive, as opposed to only the aggregate overlap area
# that the Hamming distance in criterion 2 / Section 4.4.2 reports)
# ----------------------------------------------------------------------
def explain_local_shape_preservation():
    fig, axes = plt.subplots(1, 2, figsize=(10, 5))
    for ax in axes:
        ax.set_aspect("equal", adjustable="box")
        ax.set_axis_off()
    ...
    axes[0].set_title("Local features preserved")
    ...
    axes[1].set_title("Local features lost")
    _fill_poly(axes[1], lost, COLOR_BAD, alpha=0.35)
    axes[1].plot(*base.mean(axis=0), marker="", linestyle="")
    axes[1].text(0.5, -0.04, "shape rounded into a generic blob\n(peninsula & notch no longer readable)",
                 ha="center", va="top", fontsize=9, color=COLOR_BAD, transform=axes[1].transAxes)

    # # fig.suptitle("Local shape preservation: do a region's own distinctive features survive?",
    #              fontsize=12, y=1.03)
    fig.tight_layout()
    return fig
...
# ----------------------------------------------------------------------
FIGURES = {
    "qc_cartographic_error": explain_cartographic_error,
    "qc_hamming_distance": explain_hamming_distance,
    "qc_positional_fidelity": explain_positional_fidelity,
    "qc_adjacency_error": explain_adjacency_error,
    "qc_overlap": explain_overlap,
    "qc_gap_error": explain_gap_error,
    "qc_spatial_deformation": explain_spatial_deformation,
    "qc_global_shape_preservation": explain_global_shape_preservation,
    "qc_local_shape_preservation": explain_local_shape_preservation,
    "qc_complexity": explain_complexity,
    "qc_data_ink_ratio": explain_data_ink_ratio,
}


def save_all(outdir="qc_figures", dpi=200, formats=("png", "svg")):
