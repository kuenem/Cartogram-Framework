...
def load_cartogram_data(
    ...
    for feat in features:
        ...
        if fill_with_values_from_other_years:
            continue
        if value is None:
            if verbose:
...
def _load_stat_data(
    ...
    df = loader(data_dir, detail, data_type, year, verbose=verbose)
    if verbose:
...
def _lookup_value(
    ...
    rows = df[mask]
    print(f"DataFrame in _lookup_value:\n{df}")
    if rows.empty:
...
def _load_nl_stat(data_dir: str, detail: str | None, data_type: str, year: int,
    ...
    df = df[df["Perioden"] == year].copy()
    if df.empty:
        available = sorted(pd.read_csv(path, sep=";", encoding="utf-8-sig")["Perioden"].unique())
        raise ValueError(f"Year {year} not in {path}. Available years: {available}")

    df["name"] = df["Regio's"].str.replace(r"\s*\(PV\)$", "", regex=True)
    df = df[df["name"] != "Nederland"]  # drop country total
