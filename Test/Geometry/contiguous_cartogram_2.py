...
def contiguous_cartogram(
    ...
    for iteration in range(n_iter):
        ...
        # --- soft repulsion between adjacent centroids (DCP-compliant) ---
        # Penalise when the projected centroid separation falls below the
        # sum of "radii".  We project onto the reference direction (constant)
        # so the expression stays affine in the variables → DCP-safe.
        repulsion_weight = 0.3 * W_shape
        for (i, j) in adj_set:
            ci = cp.sum(vvars[i], axis=0) / vvars[i].shape[0]   # affine
            cj = cp.sum(vvars[j], axis=0) / vvars[j].shape[0]   # affine

            # Reference direction between centroids at current iteration
            ref_ci = current_polys[i].mean(axis=0)   # constant (2,)
            ref_cj = current_polys[j].mean(axis=0)   # constant (2,)
            direction = ref_cj - ref_ci
            norm = float(np.linalg.norm(direction))
            if norm < 1e-10:
                continue
            direction = direction / norm              # unit vector (constant)

            # Projected separation along that direction — affine in variables
            proj_sep = direction @ (cj - ci)         # scalar affine expr

            # Minimum desired separation = sum of polygon "radii"
            min_sep = float(np.sqrt(areas_n[i]) + np.sqrt(areas_n[j]))

            # Hinge loss: penalise proj_sep < min_sep
            # slack >= min_sep - proj_sep  (affine RHS → DCP ✓)
            slack = cp.Variable(nonneg=True)
            constraints.append(slack >= min_sep - proj_sep)
