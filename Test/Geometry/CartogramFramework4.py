...
def CartogramFramework_global_CLAUDE_4(
    polygons,
    target_areas,
    fixed_points=None,
    shape: str = "original",
    ...
    for i, (poly, ta, fp, tc) in enumerate(
        ...
        constraints += [
            ...
            t >= t_min,                 # eq. 5  lower scale bound
            t <= t_max,                 # upper scale bound (prevents runaway vertices)
            z >= t - 1,                 # eq. 6
            z >= -(t - 1),              # eq. 7
        ]
...
if __name__ == "__main__":
    ...
    target_areas_raw = [original_areas[0], 2.0 * original_areas[1], original_areas[2]]
