...
def CartogramFramework_global(
    ...
    # ── Anti-inversion / overlap prevention ────────────────────────────────
    prevent_inversion: bool = False,
    # When True, adds a linear constraint for every adjacent pair (i,j) in T
    # that prevents their centres from swapping relative position compared to
    # the original map.  Specifically, for each adjacent pair the dot product
    # of the current centre-to-centre vector with the ORIGINAL centre-to-centre
    # vector must remain positive (plus a margin):
    #
    #   (m_j - m_i) · (c_j - c_i) >= separation_margin
    #
    # This is AFFINE in the displacement variables (delta_i, delta_j) and
    # therefore fully DCP-compliant — pure LP, no SOC needed.
    #
    # Why this helps: polygon overlap between neighbours almost always starts
    # with their centres "inverting" — j crossing to the wrong side of i.
    # Once centres invert, the shared boundary vertices are pulled in opposite
    # directions and the polygons cross.  Preventing inversion stops this at
    # the root cause.
    #
    # Only adjacent pairs (T) are constrained — O(|T|) constraints total,
    # not O(n²).  For a 48-state US map this is ~100 constraints, not 1128.
    #
    # Smart subset options via prevent_inversion_pairs:
    #   None (default) — all adjacent pairs in T
    #   "horizontal"   — only pairs that are also in H (left-right neighbours)
    #   "vertical"     — only pairs that are also in V (above-below neighbours)
    #   list[(i,j)]    — explicit custom list

    separation_margin: float = 0.0,
    # Minimum dot product value for the anti-inversion constraint.
    # 0.0 (default): centres must stay on the correct side but may touch.
    # > 0 (e.g. 1.0): enforces a minimum separation in the original direction,
    #   measured in squared-degree units (roughly: for states ~5° apart,
    #   a margin of 1.0 means centres must stay at least ~0.2° apart in
    #   the original relative direction).
    # Increase if overlaps persist; decrease if infeasibility appears.

    prevent_inversion_pairs = None,
    # Which adjacent pairs to constrain. See prevent_inversion docstring.
    ...
    # ------------------------------------------------------------------
    # 4b-pre2. Anti-inversion constraints (prevent polygon overlap)
    # ------------------------------------------------------------------
    # For each adjacent pair (i,j) enforce that their displaced centres
    # maintain the same relative orientation as in the original map:
    #
    #   (m_j - m_i) · (c_j - c_i) >= separation_margin
    #
    # = (fp_j + delta_j - fp_i - delta_i) · d0_ij >= margin
    # = d0_ij · (delta_j - delta_i) + ||d0_ij||² >= margin
    #
    # where d0_ij = fp_j - fp_i is a CONSTANT vector.
    # Every term is affine in the CVXPY variables → pure LP, DCP-compliant.
    #
    # Applied only to O(|T|) adjacent pairs — not O(n²) all pairs.
    if prevent_inversion:
        # Determine which pairs to constrain
        if prevent_inversion_pairs is None:
            inv_pairs = list(adjacent_set)
        elif prevent_inversion_pairs == "horizontal":
            inv_pairs = [(i, j) for (i, j) in adjacent_set if (i, j) in h_set or (j, i) in h_set]
        elif prevent_inversion_pairs == "vertical":
            inv_pairs = [(i, j) for (i, j) in adjacent_set if (i, j) in v_set or (j, i) in v_set]
        else:
            inv_pairs = list(prevent_inversion_pairs)

        n_inv = 0
        for (i, j) in inv_pairs:
            fp_i = np.asarray(_fp_vals[i], dtype=float)
            fp_j = np.asarray(_fp_vals[j], dtype=float)
            d0 = fp_j - fp_i                        # original direction vector (constant)
            d0_norm_sq = float(np.dot(d0, d0))

            if d0_norm_sq < 1e-10:
                continue   # coincident centres — skip

            # (m_j - m_i) · d0 = d0 · (delta_j - delta_i) + d0_norm_sq
            # Both centers_vars[i] and centers_vars[j] are affine expressions
            # (fp + delta), so their difference is affine.
            dot_expr = d0[0] * (centers_vars[j][0] - centers_vars[i][0])                      + d0[1] * (centers_vars[j][1] - centers_vars[i][1])
            # dot_expr = d0 · (delta_j - delta_i)   (affine in CVXPY vars)
            # Full dot product = dot_expr + d0_norm_sq
            constraints.append(dot_expr + d0_norm_sq >= separation_margin)
            n_inv += 1

        print(f"Anti-inversion: {n_inv} orientation constraints added "
              f"(separation_margin={separation_margin}).")
